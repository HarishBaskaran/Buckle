    ...
    public static void runner(TestCases testData, TestResult result) {
        ...
        if (response.getStatusCode() == testData.testStatusCode) {
            new AllureReporting(lifecycle)
                    .step("step-3", "Validation Details", testResult, io.qameta.allure.model.Status.PASSED, "");
        } else {
            new AllureReporting(lifecycle).updateStatus(result, io.qameta.allure.model.Status.FAILED, testResult);
            new AllureReporting(lifecycle)
