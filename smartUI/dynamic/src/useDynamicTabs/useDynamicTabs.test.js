...
describe("counting tabs renders : ", () => {
  test("each tab should be rendered once initially", () => {
    ...
    renderApp(getDeps);
    expect(_api.optionsManager.setting.panelIdTemplate.mock.calls.length).toBe(
      3
    );
  ...
  test("only current selected and previously selected tabs should be rendered after switching", () => {
    ...
    expect(_api.optionsManager.setting.panelIdTemplate.mock.calls[0][0]).toBe(
      "1"
    );
    expect(_api.optionsManager.setting.panelIdTemplate.mock.calls[1][0]).toBe(
      "2"
    );
  });
