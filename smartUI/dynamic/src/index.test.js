...
describe("apply multiple actions : ", () => {
    ...
    return act(() => {
      return instance.select("2").then(() => {
        return instance.close("2").then(() => {
          expect(
            document
              .querySelector('[tab-id="1"]')
              .className.includes("rc-dyn-tabs-selected")
          ).toBe(true);
        });
      });
    });
  ...
  test("set options and tab data + call refresh", () => {
      ...
      if (instance.getOption("direction") === "ltr") {
        ...
        instance.setOption("tabComponent", function (props) {
          return (
            <a {...props.tabProps}>
              {props.children}
              {Object.prototype.hasOwnProperty.call(props, "iconProps") && (
    ...
    expect(document.querySelector('li[tab-id="1"] .rc-dyn-tabs-close')).toBe(
      null
    );
    expect(
      document.querySelector("ul").className.includes("rc-dyn-tabs-rtl")
    ).toBe(true);
    expect(document.querySelectorAll("a.rc-dyn-tabs-title").length === 2).toBe(
      true
    );
...
describe("lazy tabs : ", () => {
  ...
  test("lazy panels should be null initially expect selected panel", () => {
    ...
    renderLazyApp();
    expect(
      document.querySelector('.rc-dyn-tabs-panel[tab-id="1"] p') !== null
    ).toBe(true);
    expect(
      document.querySelector('.rc-dyn-tabs-panel[tab-id="2"] p') !== null
    ).toBe(true);
    expect(
      document.querySelector('.rc-dyn-tabs-panel[tab-id="3"] p') === null
    ).toBe(true);
    expect(
      document.querySelector('.rc-dyn-tabs-panel[tab-id="4"] p') === null
    ).toBe(true);
    act(() => {
      instance.select("4");
      instance.select("3");
  ...
  test("selecting none existent tab", () => {
    ...
    return act(() => {
      return instance.select("3").then(() => {
        expect(document.querySelector(".rc-dyn-tabs-selected") == null).toBe(
          true
        );
...
describe("onChange callback : ", () => {
  test(`onChange is called with {currentData,previousData,perviousData,openedTabIDs,closedTabIDs} 
    ...
    expect(op.onChange.mock.calls[0][0].currentData).toEqual(
      instance.getData()
    );
    expect(op.onChange.mock.calls[0][0].previousData).toEqual(
      instance.getPreviousData()
    );
  });
  ...
  test("onChange parameters should be immutable", () => {
    ...
    const onChange1 = jest.fn(
      ({ currentData, previousData, closedTabIDs, openedTabIDs }) => {
        closedTabIDs.push("6");
        openedTabIDs.push("6");
