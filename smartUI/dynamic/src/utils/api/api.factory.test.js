...
describe("Api.prototype.open : ", () => {
  test("it should call optionsManager.validateTabData internally", () => {
    ...
    obj.open({ id: "2" });
    expect(obj.optionsManager.validateTabData.mock.calls.length === 1).toBe(
      true
    );
    expect(obj.optionsManager.validateTabData).toHaveBeenCalledBefore(
      obj._getFlushEffectsPromise
    );
...
describe("Api.prototype.ready : ", () => {
  test("ready function must be a binded function", () => {
    expect(
      Object.prototype.hasOwnProperty.call(obj.ready, "prototype") === false
    ).toBe(true);
  });
...
describe("Api.prototype._getPreSiblingTabId : ", () => {
  test("it should call helper.filterArrayUntilFirstValue with appropriate parameters", () => {
    ...
    expect(obj.helper.filterArrayUntilFirstValue.mock.calls[0][0]).toEqual([
      "1",
    ]);
