...
const {
  throwMissingParam: missingParamEr,
  isArray,
  thorwInvalidParam,
} = Helper;
export const apiConstructor = function (getDeps, param = { options: {} }) {
  const { optionsManager, helper, activedTabsHistory } = getDeps.call(
    this,
    param.options
  );
...
const _apiProps = {
  ...
  _subscribeSelectedTabsHistory: function () {
    this.on("onChange", ({ currentData, previousData, closedTabIDs }) => {
      ...
      const isSwitched =
        previousData.selectedTabID !== currentData.selectedTabID;
      if (
        isSwitched &&
        this.isOpen(previousData.selectedTabID) &&
        !this.isSelected(previousData.selectedTabID)
      )
  ...
  setTab: function (id, newData = {}) {
    this.optionsManager
      .validateObjectiveTabData(newData)
      .validatePanelComponent(newData);
...
Helper.setNoneEnumProps(_apiProps, {
  onChange: function ({
    ...
    if (isSwitched || openedTabIDs.length || closedTabIDs.length) {
      ...
      if (isSwitched) {
        if (
          newState.selectedTabID &&
          this.activedTabsHistory.tabsId.indexOf(newState.selectedTabID) === -1
        ) {
  ...
  eventHandlerFactory: function ({ e, id }) {
    ...
    if (
      ...
      this.getOption("beforeClose").call(this.userProxy, e, id) !== false &&
        this.close(id, true);
    } else {
      this.getOption("beforeSelect").call(this.userProxy, e, id) !== false &&
        this.select(id);
    }
  },
});
