...
import PopupModal from "smart/pages/hb_components/popup1";
import Input from "smart/pages/hb_components/input";
...
const FolderMenu = (props) => {
  ...
  const handleDownloadFolderPopupSave = async () => {
    try {
      ...
      const url = `http://localhost:8080/download-folder?${queryString}`;
...
const FolderItem = ({ collectionName, setCollections }) => {
  ...
  return (
    <div
      className="flex flex-row justify-between w-full hover:text-sky-600 hover:border-l-2 hover:border-sky-600 hover:cursor-pointer"
      onMouseEnter={handleMouseEnter}
      onMouseLeave={handleMouseLeave}
    >
      <p className="mx-2 hover:text-sky-600 hover:cursor-pointer w-full ">
        {collectionName}
      </p>
      {isHovered && (
        <FolderMenu
...
const RequestItem = ({ folderName, requestName, setCollections }) => {
  ...
  return (
    ...
    >
      <p
        className="ml-[20px] mr-4 hover:text-sky-600 hover:cursor-pointer w-full"
        onClick={() => {
        ...
        <div key={collectionName} className="mb-[15px] ">
          ...
          {collections[collectionName]
            ...
            .map((file) => (
              <div
                key={file.name}
                className="hover:text-sky-600 hover:border-l-2 hover:border-sky-600 hover:cursor-pointer flex justify-between"
              >
                <RequestItem
...
const runnerEndpoint = "http://localhost:8080/runCollection";
...
const dirEndpoint = "http://localhost:8080/directory";
