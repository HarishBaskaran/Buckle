...
import PopupModal from "smart/pages/hb_components/popup";
import Delete from "smart/pages/hb_components/delete_button";
import CheckBox from "smart/pages/hb_components/checkbox";
import Dropdown from "smart/pages/hb_components/dropdown_single";
import FloatingFilled from "smart/pages/hb_components/input_floating_NoChange";
import Switch from "smart/pages/hb_components/switch";
import {
  customOptionsConfig,
  dateOptions,
  dateTypeOptions,
...
const MultiQueryConfig = () => {
  ...
  let { modalIsOpen, setModalIsOpen, path, index, type } = useContext(
    multiQueryCustomTestDataContext
  );

  const { queryMultiTestConfig, setQueryMultiTestconfig } = useContext(
  ...
  const handleSelectUnique = (event, configOption) => {
    const updatedMultiTestConfig = queryMultiTestConfig.map((multiPath) => {
    ...
    setQueryMultiTestconfig(updatedMultiTestConfig);
  };
  ...
  const handleCustomValueSave = () => {
    const updatedMultiTestConfig = queryMultiTestConfig.map((multiPath) => {
      if (multiPath.name == path.name) {
        return {
          ...multiPath,
          options: multiPath.options.map((option, optionIndex) => {
            if (optionIndex == index) {
              return option.map((obj) => {
                if ("Custom" === obj.label) {
  ...
  const handleCustomValueChange = (selected, event, type) => {
    configuration = configuration.map((config) => {
      ...
      } else if (config.label == type && selected == "UniqueSelect") {
        config.unique = event.target.checked;
      }
  ...
  const customValue = () => {
    return queryMultiTestConfig.map((multiPath) => {
      if (multiPath.name === path.name) {
...
export default MultiQueryConfig;
