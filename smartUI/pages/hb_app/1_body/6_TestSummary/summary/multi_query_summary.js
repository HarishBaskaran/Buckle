...
const MultiQueryStatus = () => {
  ...
  const handleStatusChange = (e, summary) => {
    let updatedChangeSummary = multiChangeQuerySummary.map((obj) => {
      if (compareArrays(obj.tests, summary.tests)) obj.status = e.target.value;
      return obj;
    });
    setMultiChangeQuerySummary(updatedChangeSummary);
  };
        ...
        <ul className="w-[100%]">
          <li key="summary-header" className="py-2 border-2 border-gray">
            <div className="ml-[20px] grid grid-cols-7">
              {defaultFormatComponent(`TC - ${multiChangeQuerySummary.length}`)}
          ...
          {multiChangeQuerySummary &&
            multiChangeQuerySummary.map((summary, index) => {
              return (
                <li
                  key={`${summary}-${index}`}
                  className="pb-1 border-b border-gray "
                >
                  <div className="ml-[20px] grid grid-cols-7">
...
export default MultiQueryStatus;
