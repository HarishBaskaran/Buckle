@ignore
Feature: BffService - Get Bond Pdf in Aspero
  ...
  @functionality
  Scenario: Function - Bond Pdf to Aspero

    * karate.log('----------Bond Pdf.feature----------')
    ...
    * if (status == 200) karate.call('classpath:com/b2c/api/automation/functional/bffService/_16_bondPdfLController/bondPdf.feature@positive-200', { response: responseBody })
    * if (status == 500) karate.fail('The Service is down')


  @positive-200
  Scenario: Function - Verify Positive Bond Pdf
